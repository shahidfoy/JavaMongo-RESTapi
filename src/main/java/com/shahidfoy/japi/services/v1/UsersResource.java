...
import com.shahidfoy.japi.BusinessManager;
...
public class UsersResource {
	...
	public Response getUser() {
		try {
			...
			return Response.status(Response.Status.BAD_REQUEST)
					.entity("Failed: {\"error\":\"Could Not Find User\", \" status\":\"FAIL\"}")
					.build();
		}
	}
	...
	"<p><u>Input Parameters</u><li><b>new user object</b> is required</li></ul>")
	@ApiResponses(value = { @ApiResponse(code = 201, message = "Success: { user profile }"),
	@ApiResponse(code = 400, message = "Failed: {\"error\":\"Could Not Find User\", \" status\":\"FAIL\"}")})
	public Response createUser(
		@ApiParam(value = "New User", required = true, defaultValue = "\"{\"name\":\"Black Wolf\"}\"", allowableValues = "", allowMultiple = false)
		User user)
	{
		try {
	...
	@ApiResponses(value = { @ApiResponse(code = 200, message = "Success: { user_profile }"),
	@ApiResponse(code = 400, message = "Failed: {\"error\":\"error description\", \" status\":\"FAIL\"}") })
